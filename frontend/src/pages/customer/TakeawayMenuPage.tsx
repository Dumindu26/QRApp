...
export function TakeawayMenuPage() {
  ...
  // Unique allergens across cart items
  const cartAllergens = (() => {
  ...
  if (showWelcome && restaurantInfo) return (
    <WelcomeScreen
      ...
      {/* Floating cart panel */}
        <div className={`fixed bottom-0 left-0 right-0 z-50 md:left-auto md:top-[132px] md:w-72 md:border-l md:border-gray-100 md:bg-white md:z-30 ${itemCount === 0 ? 'hidden md:block' : ''}`}>
          <div className="max-w-5xl mx-auto px-4 pb-4 md:flex md:h-full md:flex-col md:px-3 md:pt-3">
            {cartOpen && (
              <div className="bg-white rounded-t-3xl shadow-2xl border border-gray-100 mb-0 max-h-[60vh] flex flex-col md:min-h-0 md:flex-1 md:rounded-2xl md:shadow-none">
                      ...
                      <li key={key} className="py-2.5">
                        <div className="flex items-center gap-3">
                          <div className="flex items-center gap-2 shrink-0">
                            ...
                            <button onClick={() => dispatch({ type: 'INC', key })} className="w-10 h-10 rounded-full bg-orange-100 text-orange-600 flex items-center justify-center hover:bg-orange-200 active:scale-95 transition-all">
                              <Plus size={14} />
                            </button>
                          </div>
                          <div className="flex-1 min-w-0">
                ...
                <div className="px-4 py-3 border-t border-gray-100">
                  ...
                  {(discount > 0 || charges.tax > 0) && (
                    <div className="mt-2 space-y-0.5 text-xs text-gray-500">
                      ...
                      {charges.tax > 0 && <div className="flex justify-between"><span>{restaurantInfo?.taxName ?? 'Tax'} ({restaurantInfo?.taxPct}%)</span><span>+{fmt(charges.tax)}</span></div>}
            ...
            >
              ...
              <span className="font-semibold">
                {placing ? t('customer.placingOrder') : itemCount === 0 ? t('customer.yourCart') : cartOpen ? t('customer.placeOrder', { amount: fmt(total) }) : t('customer.yourCart')}
              </span>
              {cartOpen
